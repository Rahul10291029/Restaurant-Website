...
const DynamicMenuCard = ({ category }) => {
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-amber-100">

      {/* Header */}
      <div className={`relative bg-gradient-to-r ${headerGradient} px-6 py-5 overflow-hidden`}>
      ...
      <div className="divide-y divide-amber-50">
        {category.items.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="group/item flex items-start justify-between gap-4 px-5 py-4 hover:bg-amber-50/70 transition-colors duration-150"
            >
              {/* Left */}
              <div className="flex items-start gap-2 flex-grow min-w-0">
                {showDot && <FoodDot type={itemType} />}
                <div className="min-w-0">
                  <span className="text-sm md:text-base font-semibold text-gray-800 group-hover/item:text-[#6b3a00] transition-colors duration-150 leading-tight block">
                    {name}
                  </span>
                  {item.description && (
                    <p className="mt-0.5 text-xs md:text-sm text-gray-400 leading-snug">
                      {item.description}
                    </p>
                  )}
                </div>
              ...
              <div className="flex-shrink-0 pt-0.5">
                <span className="inline-block px-3 py-1 rounded-full text-sm font-bold bg-amber-50 text-[#6b3a00] border border-amber-200 group-hover/item:bg-[#6b3a00] group-hover/item:text-white group-hover/item:border-transparent transition-all duration-150">
                  {item.price}
                </span>
              </div>
            </div>
